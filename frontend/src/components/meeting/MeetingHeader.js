...
export default function MeetingHeader({ meeting, participantCount }) {
  ...
  return (
    <header 
      className="bg-[#121212] border-b border-white/5 px-6 py-3.5 flex items-center justify-between gap-4 text-white z-20 select-none"
      id="meeting-header-root"
    >
      {/* Left: Security and Title */}
      <div className="flex items-center gap-3" id="meeting-header-left">
        <div 
          className="flex items-center gap-1.5 text-xs font-semibold text-emerald-400 bg-emerald-500/10 px-2.5 py-1 rounded-full border border-emerald-500/20"
          id="secure-indicator"
